...
public final class Logger {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = AppContext.getInstance()
                    .getPackageManager()
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && AppContext.getInstance().getExternalCacheDir() != null)
                sDefaultDir = AppContext.getInstance().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = AppContext.getInstance().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
